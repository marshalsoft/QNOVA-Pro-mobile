...
import { ScrollView, Text, StyleSheet, View, TouchableOpacity, Image, TextInput, BackHandler, Share, FlatList } from "react-native";
...
import { CreateNewBulkTranferProp } from "./NewBulkRequest";
type SectionProp = "Payment Request" | "Request Details" | "New Request" | "pin" | "success";
const BulkTransferDetailComponent = ({route,goBack,onSuccess}: ScreenComponentType) => {
...
<View style={{backgroundColor:"rgba(242, 242, 242, 1)",flexDirection:"column",paddingVertical:30,paddingTop:0,height:DEVICE.height,borderTopRightRadius:20,borderTopLeftRadius:20}}>
<View style={{paddingHorizontal:20}}>
<Text style={{fontSize:14,marginBottom:20,color:"rgba(123, 127, 153, 1)",textAlign:"center",width:"90%",alignSelf:"center"}}>Double-Check for Success: Confirm Your Account Details</Text>
 <Text1 style={{color:COLOURS.purple,alignSelf:"flex-start",marginBottom:16,fontWeight:"bold",fontFamily:FONTFAMILY.INTER.medium,fontSize:14}}>Beneficiaries</Text1>
 <FlatList 
 data={formData.beneficiaries}
 renderItem={({item,index})=><MockInput 
 label={item.accountName}
 value={item.isQNovaAccount?"QNOVA Pro ":item.bankName}
 component={<View style={{flexDirection:"row",alignItems:"center",gap:10}}>
    ...
    <View style={{width:5,height:5,backgroundColor:COLOURS.black,borderRadius:3}}/>
    <Text>₦{ReturnComma(String(item.amount))}</Text>
 </View>}
 />} 
 ItemSeparatorComponent={()=><View style={[Styles.divider,{marginBottom:20}]} />}
 />
  <View style={[Styles.divider,{marginBottom:20}]} />
 <Text1 style={{color:COLOURS.purple,alignSelf:"flex-start",marginBottom:16,fontWeight:"bold",fontFamily:FONTFAMILY.INTER.medium,fontSize:14}}>Payment</Text1>
 <View style={[Styles.divider,{marginBottom:20}]} />
 <MockInput 
 label={"Amount"}
 value={""}
 component={<View style={{flexDirection:"row",alignItems:"center",gap:10}}>
    <Text>₦{ReturnComma(String(formData.beneficiaries.map((a,i)=>parseFloat(String(a.amount))).reduce((a,b)=>a+b,0)))}</Text>
 </View>}
 />
  <View style={[Styles.divider,{marginBottom:20}]} />
  <MockInput 
 ...
 value={""}
 component={<View style={{flexDirection:"row",alignItems:"center",gap:10}}>
    <Text>₦0{ReturnComma(String("0"))}</Text>
 </View>}
 />
  <View style={[Styles.divider,{marginBottom:20}]} />
  <MockInput 
...
export default BulkTransferDetailComponent;
