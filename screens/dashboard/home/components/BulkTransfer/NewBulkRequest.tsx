...
const FormSchema = y.object({
    ...
    upload:y.string().required('Select a file.'),
    groupName:y.string().required('Group name is required.')
});
...
interface NewBulkRequestComponentProps {
  goBack:()=>void;
  formData:CreateNewBulkTranferProp;
  onValue:(data:any)=>void;
}
const NewBulkRequestComponent = ({goBack,formData,onValue}:NewBulkRequestComponentProps) => {
const {CreateNewBulkTranfer,loading,ShowMessage} = useHttp()
const handleBack = ()=>{
    if(goBack)
    {
    goBack();
...
initialValues={{
  accountType:"",
  upload:formData?.beneficiaries?JSON.stringify(formData.beneficiaries):"",
  groupName:formData?.groupName,
  saveBeneficiaries:false
}}
onSubmit={(values:any, actions:any) => {
  var data = values;
  data.beneficiaries = JSON.parse(values.upload);
  onValue(data);
}}
validationSchema={FormSchema}
>
{({values,handleChange,setFieldValue,handleSubmit,errors,isValid,touched})=>(<View style={{backgroundColor:"#F2F2F2",flexDirection:"column",padding:24,height:DEVICE.height,borderTopRightRadius:20,borderTopLeftRadius:20}}>
...
<BaseSelect 
 ...
 title=""
 onPress={()=>{
  SaveTemplateFile("template").then((content)=>{
    if(!content)
    {
      ShowMessage("top").fail("File not exist")
    }else{
    ShowMessage("top").success(`Template file saved to download folder.`);
  Alert.alert("Successfull!",`Template file saved to download folder. \nChoose "View file" to view. 
    `,[
    {onPress:()=>{
     DocumentPicker.showFilePicker({chooseFileButtonTitle:"File"}, (response) => {
      })
    },text:"View file"},
    {onPress:()=>{},text:"Cancel"}
  ],{cancelable:true})
...
<View style={{flex:1,flexDirection:"row",gap:5,alignItems:"center",justifyContent:"center"}}>
    <DownloadIcon />
    <Text style={{color:COLOURS.white}}>Download Beneficiary List Template</Text>
</View>
 </BaseButton>
 <Text1 style={{fontSize:12,marginTop:15}}>or</Text1>
<View 
...
>
<EmailIcon />
<Text1 style={{color:"#7B7F99",fontSize:14,marginTop:5}}>Email</Text1>
</TouchableOpacity>
<TouchableOpacity 
style={{flex:1,flexDirection:"column",alignItems:"center"}}
>
<WhatAppIcon />
<Text1 style={{color:"#7B7F99",fontSize:14,marginTop:5}}>WhatsApp</Text1>
</TouchableOpacity>
<TouchableOpacity 
onPress={()=>CopyText("jj")}
style={{flex:1,flexDirection:"column",alignItems:"center",marginVertical:20}}
>
<CopyIcon />
<Text1 style={{color:"#7B7F99",fontSize:14,marginTop:5}}>Copy</Text1>
</TouchableOpacity>
<TouchableOpacity 
style={{flex:1,flexDirection:"column",alignItems:"center"}}
>
<ShareIcon />
<Text1 style={{color:"#7B7F99",fontSize:14,marginTop:5}}>Share</Text1>
</TouchableOpacity>
</View>

<Card 
style={{height:62,borderRadius:12,alignItems:"center"}}
>
<View style={{flex:1,justifyContent:"flex-start",width:"100%"}}>
<MockInput 
 value={values.upload?JSON.parse(values.upload).map((a:any,i:number)=>{
  return a.accountName
 }).join(", "):`Tap to upload`}
 label="Beneficiary List (.csv, 2mb max)"
 />
 </View>
 <TouchableOpacity 
 onPress={()=>{
    ...
    if(res.status)
    {
      ...
      {
        const getKeys = Object.keys(res.data[0]).join(" ");
        if(titleKeys.join(" ") !== getKeys)
        {
          ShowMessage("top").fail(`Please upload the correct template`);
        }else{
        setFieldValue("upload",JSON.stringify(res.data)); 
        }
      }else{
...
<Text style={[AppStyles.error,{marginBottom:0}]}>{errors.upload}</Text>
 <View 
style={{height:62,borderRadius:12,marginBottom:20,alignItems:"center",flexDirection:"row"}}
>
<View style={{flex:1,justifyContent:"flex-start",width:"100%"}}>
<Text1 style={{alignSelf:"flex-start",color:COLOURS.black}}>Save Beneficiary List</Text1>
 </View>
 <BaseSwitch
 value={values.saveBeneficiaries}
 onValueChange={(d)=>{
setFieldValue("saveBeneficiaries",d);
 }}
 />
 </View>
 <BaseInput 
 ...
 onChange={(data)=>{
    setFieldValue("groupName",data)
 }}
 max={50}
 errorMessage={touched.groupName && errors.groupName}
 />
 <BaseButton 
 title="Continue"
...
export default NewBulkRequestComponent;
