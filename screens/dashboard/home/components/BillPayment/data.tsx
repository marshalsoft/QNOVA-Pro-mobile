...
const DataComponent = (props:AirtimeComponentProps)=>{
   const [selected,setSelected] = useState<number>(0);
   const [plans,setPlans] = useState<PlansProps[]>([]);
   const {VerifyAirtimeNumber,loading,ShowMessage,GetDataPlans} = useHttp();
   useEffect(()=>{
   ...
   style={{marginTop:20}}
    ...
    list={plans.map((a,i)=>{
      return {
         title:`${a.name} (NGN${ReturnComma(a.price)})`,
         value:a.price,
         currency:a.datacode
      }
    })}
    ...
    onChange={(data)=>{
      setFieldValue("amount",parseFloat(String(data.value).replace(/[, ]/g,'')));
      setFieldValue("planName",data.title);
      setFieldValue("dataCode",data.currency);
    }}
    type="custom"
